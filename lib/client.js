window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const ICONS = {
			...
			battery: [
				'M2.6 5.2h8.1c.6 0 1.1.5 1.1 1.1v3.4c0 .6-.5 1.1-1.1 1.1H2.6c-.6 0-1.1-.5-1.1-1.1V6.3c0-.6.5-1.1 1.1-1.1z',
				'M13.6 7.1v1.8',
				'M3.6 6.8h3.8v2.4H3.6z',
			],
...
/* 电量卡和网络卡同宽；卡片窄到放不下时按 .ddm-main / .ddm-stat 的省略号规则收缩。 */
.ddm-row2 > .ddm-battery { flex: 0 1 250px; min-width: 190px; }
...
.ddm-main { display: flex; align-items: baseline; gap: 8px; margin-top: 3px; min-width: 0; }
/* 大号读数旁边的小注解（电量卡的充电状态）。 */
.ddm-note {
  min-width: 0;
  overflow: hidden;
...
`
		...
		/** 电量偏低时才着色：和大数值、底栏共用一套阈值。 */
		function batteryClass(percent) {
			if (typeof percent !== 'number') return ''
			if (percent <= 15) return 'ddm-crit'
			if (percent <= 30) return 'ddm-hot'
			return ''
		}

		/** 内核给的状态字翻成界面语言；认不出来就把原文显示出来。 */
		...
		/** 充电提示：只有真的在充或者已经充满才值得占位置。 */
		function batteryNote(battery) {
			if (battery.charging) return { text: `⚡ ${t.charging}`, live: true }
			if (battery.full) return { text: t.full, live: false }
			if (battery.plugged) return { text: `⚡ ${t.notCharging}`, live: false }
			return null
		}

		function formatVolts(volts) {
			if (typeof volts !== 'number' || !Number.isFinite(volts)) return '—'
			return `${volts.toFixed(2)} V`
		}

		/** 电流统一显示成毫安，超过 1A 才换安培；放电是负数。 */
		function formatMilliAmps(milliAmps) {
			if (typeof milliAmps !== 'number' || !Number.isFinite(milliAmps)) return '—'
			const magnitude = Math.abs(milliAmps)
			if (magnitude >= 1000) return `${(milliAmps / 1000).toFixed(2)} A`
			if (magnitude >= 10) return `${Math.round(milliAmps)} mA`
			if (magnitude >= 1) return `${milliAmps.toFixed(1)} mA`
			return '0 mA'
		}
		...
		function MetricCard(props) {
			...
			return React.createElement(
				'section',
				{
					className: props.cardClass === undefined ? 'ddm-card' : `ddm-card ${props.cardClass}`,
					style: { '--ddm-accent': props.accent },
				},
				React.createElement(
					'div',
				...
				React.createElement(
					'div',
					{ className: 'ddm-main' },
					React.createElement('span', { className: `ddm-big ${props.valueClass ?? ''}` }, props.value),
					props.note === undefined || props.note === null
						? null
						: React.createElement('span', { className: `ddm-note ${props.noteClass ?? ''}` }, props.note),
				),
		...
		function DeviceMonitor() {
			...
			const segments = [
				...
				battery === null
					...
					: React.createElement('span', {
							key: 'battery',
							className: `ddm-seg ${batteryClass(battery.percent)}`,
							title:
								`${batteryStatusText(battery.status)} · ${formatTemp(battery.tempC)} · ${formatVolts(battery.voltageV)} · ` +
								`${t.current} ${formatMilliAmps(battery.currentMa)}`,
						}, `${t.battery} ${formatPercent(battery.percent, 0)}${battery.charging ? ' ⚡' : battery.full ? ' ⚡' : ''}`),
				React.createElement('span', {
					key: 'net',
					className: 'ddm-seg',
			...
			return React.createElement(
				...
				open
					? renderPanel({ cpu, gpu, memory, battery, storage, net, series, procs, status })
					: null,
			)
		}

		/** 展开面板：外层标题栏 + 第一行四张指标卡 + 第二行电量/网络/进程卡。 */
		function renderPanel(state) {
			const { cpu, gpu, memory, battery, storage, net, series, procs, status } = state
			...
			// ── 电量 ─────────────────────────────────────────
			// 没有电池的机器（台式机、容器）整块不出现，而不是显示 0%。
			const batteryNoteText = battery === null ? null : batteryNote(battery)
			const batteryCard = battery === null ? null : React.createElement(MetricCard, {
				key: 'battery',
				cardClass: 'ddm-battery',
				// 电量低到该充电时进度条跟着变红/变橙。
				accent: batteryClass(battery.percent) === 'ddm-crit' ? '#e5484d' : batteryClass(battery.percent) === 'ddm-hot' ? '#e8871e' : ACCENT.battery,
				icon: 'battery',
				name: t.battery,
				sub: battery.technology ?? null,
				subTitle: `${t.cycles} ${battery.cycleCount ?? '—'} · ${t.health} ${battery.health ?? '—'}`,
				value: formatPercent(battery.percent, 0),
				valueClass: batteryClass(battery.percent),
				note: batteryNoteText === null ? null : batteryNoteText.text,
				noteClass: batteryNoteText !== null && batteryNoteText.live ? 'ddm-note-live' : '',
				percent: battery.percent,
				series: null,
				stats: [
					React.createElement(Stat, { key: 'temp', label: t.temp, value: formatTemp(battery.tempC), valueClass: tempClass(battery.tempC) }),
					React.createElement(Stat, { key: 'voltage', label: t.voltage, value: formatVolts(battery.voltageV) }),
					React.createElement(Stat, { key: 'current', label: t.current, value: formatMilliAmps(battery.currentMa) }),
					React.createElement(Stat, { key: 'cycles', label: t.cycles, value: battery.cycleCount === null ? '—' : String(battery.cycleCount) }),
				],
			})
			...
			return React.createElement(
				...
				React.createElement(
					...
					React.createElement('div', { className: 'ddm-row2' }, batteryCard, netCard, procCard),
				),
			)
		}
		//#endregion
