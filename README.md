...
| 电量 | 电量百分比、充电状态（充电中/放电中/已充满/未充电）、是否插电、温度、电压、电流、循环次数、健康度 | `/sys/class/power_supply/*`（按 `type=Battery` 认节点） |
...
- 输入框下方的监控栏显示：`CPU 12% 38℃ │ GPU 0% 37℃ │ 内存 36% │ 交换 12% │ 存储 53% │ 电量 42% │ 网络 ↓1.2M/s ↑24K/s`。
  - 电量 ≤30% 变橙色、≤15% 变红色；正在充电或已充满时跟一个 `⚡`。
...
- 点击监控栏展开面板，结构是：
  ...
  - **第二行**：**电量卡**（大号电量 + 充电状态注记 + 进度条，底部两列小指标：温度/电压/电流/循环次数）、网络卡（下行/上行大号读数 + 绿色/橙色曲线 + 底部总下载/总上传）与**进程内存排行**卡并排，进程卡吃掉剩余宽度。
    - 电量卡的进度条颜色随电量走（正常绿、≤30% 橙、≤15% 红），副标题是电池类型（`Li-ion`），悬停能看到循环次数与健康度。
    - 没有电池的机器（容器、台式机）这一块整个不出现，而不是显示 0%；状态字取内核原文并翻成中英文，认不出来就原样显示。
- 进程排行取前 12 名，固定高度可滚动：
